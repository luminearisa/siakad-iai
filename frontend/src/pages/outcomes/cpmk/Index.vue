...
import { Plus, Sparkles, Edit, Trash2, Search, CalendarDays, BookOpenCheck } from 'lucide-vue-next'
...
const cpmkList = ref<CourseLearningOutcome[]>([])
...
const itemToDelete = ref<CourseLearningOutcome | null>(null)

async function loadData() {
  loading.value = true
  try {
    const [cpmkRes, prodiRes] = await Promise.all([
      outcomeService.listCpmk(),
      academicService.getStudyPrograms({ per_page: 100 }),
    ])
    cpmkList.value = cpmkRes.data || []
    studyPrograms.value = prodiRes.data || []
  } catch (err: any) {
    toast.error(err.message || 'Gagal memuat data CPMK.')
  } finally {
    loading.value = false
  }
}

const filteredCpmks = computed(() => {
  let list = cpmkList.value
  ...
  if (search.value.trim()) {
    ...
    list = list.filter(
      (c) =>
        ...
        (c.course && c.course.name.toLowerCase().includes(q))
    )
  }
  return list
})

const paginatedCpmks = computed(() => {
  return filteredCpmks.value.slice(0, perPage.value)
})

function openDeleteModal(item: CourseLearningOutcome) {
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await outcomeService.deleteCpmk(itemToDelete.value.id)
    toast.success(`CPMK ${itemToDelete.value.code} berhasil dihapus.`)
    deleteModalOpen.value = false
    itemToDelete.value = null
    await loadData()
  } catch (err: any) {
    toast.error(err.message || 'Gagal menghapus CPMK.')
  } finally {
  ...
  <PageContainer>
    <div class="space-y-4">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-4">
        <div>
          <div class="flex items-center gap-2 text-xs text-slate-500 font-medium mb-1">
            ...
            <span>&rsaquo;</span>
            <span class="text-slate-900 font-semibold">Capaian Pembelajaran Mata Kuliah (CPMK)</span>
          </div>
          <h1 class="text-xl font-bold text-slate-900 tracking-tight">Capaian Pembelajaran Mata Kuliah (CPMK)</h1>
          <p class="text-xs text-slate-500 mt-0.5">Manajemen Capaian Pembelajaran Mata Kuliah (CPMK)</p>
        </div>
        ...
        <div class="flex items-center gap-2 self-start sm:self-auto">
          ...
          <Button
            ...
            @click="router.push('/outcomes/cpmk/create')"
          >
            <Plus class="w-4 h-4" />
            Tambah Data
          </Button>
      ...
      <Card class="bg-white border border-slate-200 rounded-xl shadow-2xs overflow-hidden">
        ...
        <div v-else-if="filteredCpmks.length === 0" class="py-12 text-center text-slate-400 text-xs">
          <BookOpenCheck class="w-8 h-8 text-slate-300 mx-auto mb-2" />
          <p class="font-bold text-slate-700">Belum ada data CPMK</p>
          ...
          <table class="w-full text-left text-xs border-collapse">
            <thead>
              <tr class="bg-slate-50/80 border-b border-slate-200 text-3xs font-semibold text-slate-600 uppercase tracking-wider">
                ...
                <th class="py-3 px-4 min-w-[320px]">CPMK &uarr;&darr;</th>
            ...
            <tbody class="divide-y divide-slate-100">
              <tr
                v-for="(item, index) in paginatedCpmks"
                :key="item.id"
                ...
                <td class="py-3 px-4 text-center">
                  <div class="flex items-center justify-center gap-1">
                    <button
                      ...
                      title="Edit CPMK"
                      @click="router.push(`/outcomes/cpmk/${item.id}/edit`)"
                    >
        ...
        <div class="p-3 border-t border-slate-100 bg-slate-50/40 text-2xs text-slate-500 flex items-center justify-between">
          <span>Menampilkan 1 hingga {{ paginatedCpmks.length }} dari total {{ filteredCpmks.length }} data</span>
        </div>
      </Card>
    </div>
    ...
    <ConfirmModal
      ...
      title="Hapus CPMK"
      :message="`Apakah Anda yakin ingin menghapus CPMK ${itemToDelete?.code}?`"
