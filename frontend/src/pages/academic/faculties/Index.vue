<script setup lang="ts">
import { ref, reactive, computed, onMounted } from 'vue'
import { Edit2, Trash2, X, Check, Plus } from 'lucide-vue-next'
import { academicService } from '@/services/api/academic'
import { useToast } from '@/composables/useToast'
import type { Faculty, Institution } from '@/types/academic'
import PageContainer from '@/components/data-display/PageContainer.vue'
import Card from '@/components/ui/Card.vue'
import Button from '@/components/ui/Button.vue'
import Input from '@/components/ui/Input.vue'

const toast = useToast()
const loading = ref<boolean>(false)
const faculties = ref<Faculty[]>([])
const institutions = ref<Institution[]>([])
const search = ref<string>('')
const perPage = ref<number>(10)
const drawerOpen = ref<boolean>(false)
const editingId = ref<number | null>(null)
const saving = ref<boolean>(false)

const form = reactive({
  institution_id: null as number | null,
  code: '',
  name: '',
  name_en: '',
  status: 'active',
})

const filteredFaculties = computed(() => {
  if (!search.value) return faculties.value
  const q = search.value.toLowerCase()
  return faculties.value.filter(
    (f) =>
      f.name.toLowerCase().includes(q) ||
      f.code.toLowerCase().includes(q) ||
      (f.name_en && f.name_en.toLowerCase().includes(q))
  )
})

async function fetchFaculties() {
  loading.value = true
  try {
    const res = await academicService.getFaculties()
    faculties.value = res.data || []
  } catch (err: any) {
    toast.error(err.response?.data?.message || 'Gagal memuat data fakultas')
  } finally {
    loading.value = false
  }
}

async function loadInstitutions() {
  try {
    const res = await academicService.getInstitutions()
    institutions.value = res.data || []
  } catch {
    institutions.value = []
  }
}

function openCreateDrawer() {
  editingId.value = null
  form.institution_id = institutions.value.length > 0 ? institutions.value[0].id : null
  form.code = ''
  form.name = ''
  form.name_en = ''
  form.status = 'active'
  drawerOpen.value = true
}

function openEditDrawer(item: Faculty) {
  editingId.value = item.id
  form.institution_id = item.institution_id || (institutions.value.length > 0 ? institutions.value[0].id : null)
  form.code = item.code
  form.name = item.name
  form.name_en = item.name_en || ''
  form.status = item.status || 'active'
  drawerOpen.value = true
}

function closeDrawer() {
  drawerOpen.value = false
}

async function handleSave() {
  if (!form.code || !form.name) {
    toast.error('Kode Fakultas dan Nama Fakultas wajib diisi (*)')
    return
  }

  saving.value = true
  try {
    if (editingId.value) {
      await academicService.updateFaculty(editingId.value, form)
      toast.success('Data fakultas berhasil diperbarui')
    } else {
      await academicService.createFaculty(form)
      toast.success('Data fakultas berhasil ditambahkan')
    }
    drawerOpen.value = false
    fetchFaculties()
  } catch (err: any) {
    toast.error(err.response?.data?.message || 'Gagal menyimpan data fakultas')
  } finally {
    saving.value = false
  }
}

async function handleDelete(item: Faculty) {
  if (!confirm(`Apakah Anda yakin ingin menghapus fakultas "${item.name}"?`)) return
  try {
    await academicService.deleteFaculty(item.id)
    toast.success('Fakultas berhasil dihapus')
    fetchFaculties()
  } catch (err: any) {
    toast.error(err.response?.data?.message || 'Gagal menghapus fakultas')
  }
}

onMounted(() => {
  fetchFaculties()
  loadInstitutions()
})
</script>

<template>
  <PageContainer>
    <!-- Header (Matching Image 2) -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <nav class="flex items-center gap-1.5 text-xs text-slate-400 mb-1">
          <span class="text-brand-600 font-medium">Akademik</span>
        </nav>
        <h1 class="text-xl font-bold text-slate-900">Fakultas</h1>
        <p class="text-xs text-slate-500 mt-0.5">Manajemen data fakultas di perguruan tinggi</p>
      </div>

      <Button
        variant="primary"
        size="sm"
        class="bg-brand-700 hover:bg-brand-800 text-white flex items-center gap-1.5 self-start sm:self-auto shadow-2xs font-semibold"
        @click="openCreateDrawer"
      >
        <Plus class="w-4 h-4" />
        Tambah Fakultas
      </Button>
    </div>

    <!-- Main Card -->
    <Card class="border border-slate-200/80 shadow-2xs overflow-hidden">
      <!-- Toolbar -->
      <div class="p-4 border-b border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50/50">
        <div class="flex items-center gap-2 w-full sm:w-auto text-xs text-slate-600">
          <span>Baris</span>
          <select
            v-model="perPage"
            class="px-2.5 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-medium focus:ring-2 focus:ring-brand-500/20 focus:border-brand-600 outline-none"
          >
            <option :value="10">10</option>
            <option :value="25">25</option>
            <option :value="50">50</option>
          </select>
        </div>

        <div class="flex items-center gap-2 w-full sm:w-auto">
          <div class="relative w-48 sm:w-64">
            <Input
              v-model="search"
              placeholder="Cari data..."
              class="w-full text-xs pr-8"
            />
          </div>
          <Button variant="primary" size="sm" class="bg-brand-700 hover:bg-brand-800 text-white text-xs px-3">
            Cari
          </Button>
        </div>
      </div>

      <!-- Table (Matching Image 2) -->
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse text-xs">
          <thead>
            <tr class="bg-slate-100/80 border-b border-slate-200/80 text-slate-600 font-bold uppercase tracking-wider text-3xs">
              <th class="py-3 px-4 w-12 text-center">NO</th>
              <th class="py-3 px-4 w-40">KODE FAKULTAS</th>
              <th class="py-3 px-4">NAMA FAKULTAS</th>
              <th class="py-3 px-4">NAMA FAKULTAS (INGGRIS)</th>
              <th class="py-3 px-4 text-center w-32">STATUS AKTIF</th>
              <th class="py-3 px-4 w-24 text-center">AKSI</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100 text-slate-700">
            <tr v-if="loading" class="hover:bg-transparent">
              <td colspan="6" class="py-12 text-center text-slate-400">
                Memuat data fakultas...
              </td>
            </tr>
            <tr v-else-if="filteredFaculties.length === 0" class="hover:bg-transparent">
              <td colspan="6" class="py-12 text-center text-slate-400">
                Belum ada data fakultas
              </td>
            </tr>
            <tr
              v-for="(item, idx) in filteredFaculties.slice(0, perPage)"
              :key="item.id"
              class="hover:bg-slate-50/80 transition-colors"
            >
              <td class="py-3.5 px-4 text-center font-medium text-slate-500">
                {{ idx + 1 }}
              </td>
              <td class="py-3.5 px-4 font-mono font-medium text-slate-900">
                {{ item.code }}
              </td>
              <td class="py-3.5 px-4 font-bold text-slate-900">
                {{ item.name }}
              </td>
              <td class="py-3.5 px-4 text-slate-600 italic">
                {{ item.name_en || '--' }}
              </td>
              <td class="py-3.5 px-4 text-center">
                <span
                  class="inline-flex items-center justify-center px-4 py-1 rounded text-3xs font-bold text-white shadow-2xs"
                  :class="item.status === 'active' ? 'bg-emerald-600' : 'bg-slate-400'"
                >
                  {{ item.status === 'active' ? 'Aktif' : 'Non-Aktif' }}
                </span>
              </td>
              <td class="py-3.5 px-4">
                <div class="flex items-center justify-center gap-1.5">
                  <button
                    type="button"
                    class="p-1.5 text-rose-600 hover:bg-rose-50 rounded-md border border-rose-200 transition-colors"
                    title="Edit Fakultas"
                    @click="openEditDrawer(item)"
                  >
                    <Edit2 class="w-3.5 h-3.5" />
                  </button>
                  <button
                    type="button"
                    class="p-1.5 text-rose-600 hover:bg-rose-50 rounded-md border border-rose-200 transition-colors"
                    title="Hapus Fakultas"
                    @click="handleDelete(item)"
                  >
                    <Trash2 class="w-3.5 h-3.5" />
                  </button>
                </div>
              </td>
            </tr>
          </tbody>
        </table>
      </div>

      <!-- Footer -->
      <div class="p-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 bg-white">
        <span>Menampilkan 1 hingga {{ Math.min(perPage, filteredFaculties.length) }} dari total {{ filteredFaculties.length }} data</span>
        <div class="flex items-center gap-1">
          <span class="px-2.5 py-1 rounded bg-brand-700 text-white font-bold">1</span>
        </div>
      </div>
    </Card>

    <!-- Slide-over Drawer -->
    <div
      v-if="drawerOpen"
      class="fixed inset-0 z-50 flex justify-end bg-slate-900/40 backdrop-blur-xs transition-opacity"
      @click.self="closeDrawer"
    >
      <div class="w-full max-w-md bg-white shadow-2xl h-full flex flex-col animate-slide-left">
        <div class="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <h3 class="text-sm font-bold text-slate-900">
              {{ editingId ? 'Edit Fakultas' : 'Tambah Fakultas' }}
            </h3>
            <p class="text-3xs text-slate-500 mt-0.5">Kelola unit kerja fakultas perguruan tinggi</p>
          </div>
          <button
            type="button"
            class="p-1 rounded-md text-slate-400 hover:text-slate-600 hover:bg-slate-100"
            @click="closeDrawer"
          >
            <X class="w-4 h-4" />
          </button>
        </div>

        <div class="p-5 overflow-y-auto flex-1 space-y-4 text-xs">
          <div>
            <label class="block font-semibold text-slate-700 mb-1.5">
              Kode Fakultas <span class="text-rose-500">*</span>
            </label>
            <Input
              v-model="form.code"
              placeholder="Contoh: FDIK1010 / FTI1011"
              required
            />
          </div>

          <div>
            <label class="block font-semibold text-slate-700 mb-1.5">
              Nama Fakultas (Indonesia) <span class="text-rose-500">*</span>
            </label>
            <Input
              v-model="form.name"
              placeholder="Contoh: Fakultas Desain dan Industri Kreatif"
              required
            />
          </div>

          <div>
            <label class="block font-semibold text-slate-700 mb-1.5">
              Nama Fakultas (Inggris)
            </label>
            <Input
              v-model="form.name_en"
              placeholder="Contoh: Faculty of Design and Creative Industries"
            />
          </div>

          <div>
            <label class="block font-semibold text-slate-700 mb-1.5">
              Status Aktif
            </label>
            <select
              v-model="form.status"
              class="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-xs focus:ring-2 focus:ring-brand-500/20 focus:border-brand-600 outline-none"
            >
              <option value="active">Aktif</option>
              <option value="inactive">Non-Aktif</option>
            </select>
          </div>
        </div>

        <div class="p-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-2">
          <Button variant="outline" size="sm" @click="closeDrawer">
            Batal
          </Button>
          <Button
            variant="primary"
            size="sm"
            :loading="saving"
            class="bg-brand-700 hover:bg-brand-800 text-white flex items-center gap-1.5"
            @click="handleSave"
          >
            <Check class="w-4 h-4" />
            Simpan Data
          </Button>
        </div>
      </div>
    </div>
  </PageContainer>
</template>
