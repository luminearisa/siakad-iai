...
const form = reactive({
  ...
  status: 'active' as 'active' | 'completed' | 'inactive' | 'pending_approval',
  title_id: '',
  title_en: '',
  topic_id: '',
  topic_en: '',
...
async function loadData() {
  loading.value = true
  try {
    const [thesisRes, studRes, lecRes, semRes] = await Promise.all([
      thesisService.get(thesisId),
    ...
    if (t) {
      ...
      form.start_date = t.start_date || '2026-08-26'
      form.start_semester_id = t.start_semester_id || null
      form.submission_date = t.submission_date || '2026-08-26'
      form.status = t.status
      form.title_id = t.title_id
      form.title_en = t.title_en || ''
      form.topic_id = t.topic_id || ''
      form.topic_en = t.topic_en || ''
      form.proposal_file_path = t.proposal_file_path || ''
      form.completion_date = t.completion_date || ''
      form.completion_semester_id = t.completion_semester_id || null
      form.sk_date = t.sk_date || ''
      form.sk_number = t.sk_number || ''
      form.final_file_path = t.final_file_path || ''

      if (t.supervisors && t.supervisors.length > 0) {
        form.supervisor_1_id = t.supervisors[0].lecturer_id
        if (t.supervisors.length > 1) {
          form.supervisor_2_id = t.supervisors[1].lecturer_id
        }
      }
    }
  } catch (err: any) {
...
async function handleSubmit() {
  ...
  try {
    ...
    await thesisService.update(thesisId, {
    ...
    toast.success('Data tugas akhir berhasil diperbarui!')
    router.push('/thesis')
  } catch (err: any) {
    toast.error(err.response?.data?.message || 'Gagal memperbarui data tugas akhir')
  } finally {
    saving.value = false
  }
}
...
<template>
  <PageContainer>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <nav class="flex items-center gap-1.5 text-xs text-slate-400 mb-1">
          <span class="text-brand-600 font-semibold tracking-wider uppercase">EDIT</span>
        </nav>
        <h1 class="text-xl font-bold text-slate-900">Edit Tugas Akhir</h1>
      </div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <!-- Section 3: Penyelesaian Tugas Akhir -->
      <Card
